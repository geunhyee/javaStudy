package ch14_jdbc.ch14_jdbc_conn.jdbc;
// connectionfactory 로부터 maxConn 수량만큼
// connection 객체를 미리 생성해서 풀(당구)에 보관 관리하는 클래스
...
public class ConnectionPool {
   ...
   private ConnectionPool() {
      try {
         initPool();      
         // 클래스를 인스턴스화 할 때 initPool()을 호출하여 풀을 초기화 
   ...
   // 풀에서 사용가능한 데이터베이스 연결을 가져오는 메서드
   // 가능한 연결이 없을경우 wait() 호출
   public synchronized Connection getConnection() {
      if (pool.size() == 0) {
         try {
            System.out.println("가능한 연결이 없어요... wait..please...");
            wait();
   ...
   // 연결을 풀에 추가하고, 대기중인 다른 스레드가 있다면 notify()로 사용
   public synchronized void releaseConnection(Connection conn) {
      System.out.println("사용 후 pool에 담기");
      pool.add(conn);
      notify();   // -> wait 걸려있던 데이터 다시 들어오도록?
   }
   ...
   private synchronized void initPool() throws SQLException {
      ...
      for(int i=0; i <maxConn; i++) {
         ...
         // 프로퍼티 maxConn 수량만큼 -> 세션의 갯수 || maxConn = 4 => 동시접속 4개까지 
      }
   }
