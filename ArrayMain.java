...
public class ArrayMain {
	...
	public static void main(String[] args) {
		...
		//배열에 값 넣기
		seoyugi[0] = samjang; //0번째에 samjang 이라는 value 값이 들어감
		System.out.println(seoyugi[0]);
		seoyugi[1] = woogong;
		seoyugi[2] = samjang;
		seoyugi[3] = palgye;
		System.out.println(seoyugi); //출력하면 알수없는 글자들이 나옴 뒤에있는게 주소임
		// 0부터 array.length 미만까지
		printArr(seoyugi);
		for(int i = 0; i < seoyugi.length; i++) { //length 전까지 돌아야 오류가 안남 =같다는 등호 들어가면 안됨
			seoyugi[i]= "서유기 " + seoyugi[i];
		}
		printArr(seoyugi);
		// d오공을 찾아서 -> 드래곤볼 오공으로 변경
	...
	//값을 넣으며 배열 선언
		...
		printArr(student);
		String subject = "자바, SQL, 화면구현, JSP";
	//.split(문자열);
	//대상문자열을 문자열 기준으로 나누어 배열로 리턴
		String[] subArr = subject.split(","); // , 기준으로 잘라서 배열로 바뀌는것
		printArr(subArr);
	...
	//배열의 복사(shallow copy 얕은 복사) 주소값 복사
	...
	//clone 까먹었다
		...
		printArr(numArr);
		Arrays.sort(numArr);//Arrays 임포트 -> sort 메소드사용: 오름차순 정렬
		printArr(numArr);
